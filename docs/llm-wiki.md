...
- **Word(.docx)** … MS Teamsトランスクリプトのエクスポート。話者名・タイムスタンプ・本文が並ぶ標準レイアウトを想定し python-docx で抽出。**構造は初回の実サンプルで検証（→ §10）**。開くパスワードで暗号化されたdocxの復号は未対応（pptxのような`--password`対応は未実装）。IRM/Azure RMS保護は§7.2と同じ理由で検出のみ行う
