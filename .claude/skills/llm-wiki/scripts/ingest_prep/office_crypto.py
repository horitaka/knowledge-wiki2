...
IRM_GUIDANCE = (
    ...
    "（--password 等を指定しても解決しません）。次のいずれかで対処してください。"
    "(1) ラベル/保護を解除する権限を持つ人がOffice上でファイルを開き、"
    "情報保護ラベルの解除または「アクセス制限」の解除を行った上で、"
    "保護なしの複製をraw/に配置し直してから再実行する。"
    "(2) ラベルのポリシーで印刷/エクスポートが許可されている場合は、"
    "閲覧権限を持つ人がOffice上でファイルをPDFとして保存し、"
    "そのPDFをraw/decks/（議事録の場合はraw/transcripts/）に配置し直した上で、"
    "scripts/ingest_prep/pdf_extract.py で再実行する。"
)
